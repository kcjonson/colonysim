...
// Use World.cpp (formerly ChunkedWorld) instead.
...
WorldOld::WorldOld(GameState& gameState, const std::string& seed, Camera* cam, GLFWwindow* win)
    : gameState(gameState),
      seed(seed),
...
glm::vec4 WorldOld::getCameraBounds() const {
    if (!camera) {
...
void WorldOld::render() {
    try {
        ...
        if (terrainData.empty()) {
            std::cerr << "WARNING: No terrain data available in WorldOld::render" << std::endl;
        }
        ...
        std::cerr << "ERROR: Exception in WorldOld::render: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "ERROR: Unknown exception in WorldOld::render" << std::endl;
    }
}

// Helper function to check if camera position or projection changed
bool WorldOld::cameraViewChanged() const {
...
void WorldOld::updateTileVisibility() {
...
void WorldOld::logMemoryUsage() const {
...
void WorldOld::setTerrainData(const std::unordered_map<WorldGen::TileCoord, WorldGen::TerrainData>& data) {
    terrainData = data;
    tiles.clear();
