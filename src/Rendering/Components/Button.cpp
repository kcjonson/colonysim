...
Button::Button(const ButtonArgs& args)
    ...
    // If hover style was default-initialized (e.g., color is the default sentinel),
    // create a slightly lighter version of the normal style.
    // This preserves the original logic where a default-constructed hoverStyle parameter triggered derivation.
    if (this->hoverStyle.color == glm::vec4(1.0f)) { // Assuming glm::vec4(1.0f) is the sentinel for default color
    ...
    if (this->pressedStyle.color == glm::vec4(1.0f)) { // Assuming glm::vec4(1.0f) is the sentinel for default color
    ...
    background = std::make_shared<Shapes::Rectangle>(
        ...
        this->size,           // Size
        buttonToRectangleStyle(this->style),  // Use the (potentially now final) normal style
        backgroundZ
    );
...
void Button::updateTextPosition() {
    if (labelText) {
        ...
        // Adjust position to place text in the center
        glm::vec2 textPos = centerPos;
        
        // Update the text position
        labelText->setPosition(textPos);
    }
}
...
void Button::handleMouseButton(int button, int action) {
    ...
    // If mouse is released over the button and was previously pressed, trigger click
...
void Button::handleInput(float deltaTime) {
    // This method should be called by the parent layer/system each frame
    // It processes the input from GLFW for this button
    ...
    // Check if we need to transform coordinates (e.g., from screen to world space)
    // This would depend on your rendering system
