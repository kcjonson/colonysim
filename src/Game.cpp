...
Game::Game() 
    ...
    Entity* resource = entityManager.getEntity(resourceId);
    if (resource) {
    ...
    size_t buildingId = entityManager.createEntity(
        glm::vec2(-100.0f, -100.0f),
        glm::vec2(50.0f, 50.0f),
        glm::vec4(0.5f, 0.5f, 0.5f, 1.0f) // Gray
    );
    Entity* building = entityManager.getEntity(buildingId);
    if (building) {
...
void Game::render() {
    ...
    // Render world with camera matrices
